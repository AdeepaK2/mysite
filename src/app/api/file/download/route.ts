...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (fileName) {
      ...
      try {
        ...
        if (fileUrl.includes('r2.cloudflarestorage.com')) {
          const bucketName = process.env.R2_BUCKET_NAME || "mysite";
          ...
          // Handle case where bucket name appears in the path
          if (pathname.startsWith(bucketName + '/')) {
            // Remove the bucket name from the start of the path
            pathname = pathname.substring(bucketName.length + 1);
          }
          
          // Handle double bucket name case (e.g., /mysite/mysite/projects/file.png)
          if (pathname.startsWith(bucketName + '/')) {
            pathname = pathname.substring(bucketName.length + 1);
          }
    ...
    console.log("Using bucket:", process.env.R2_BUCKET_NAME || "mysite");

    // Try with the key as provided
    let command = new GetObjectCommand({
      Bucket: process.env.R2_BUCKET_NAME || "mysite",
      Key: key,
    });
